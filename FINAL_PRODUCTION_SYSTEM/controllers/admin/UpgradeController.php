...
define('GITHUB_CACHE_TTL', 600); // 10 minutes

function fetchGitHubRelease(bool $forceRefresh = false): array {
    $cacheFile = sys_get_temp_dir() . '/oem_github_release_cache.json';

    // Check cache
    if (!$forceRefresh && file_exists($cacheFile)) {
        $cached = json_decode(file_get_contents($cacheFile), true);
        if ($cached && (time() - ($cached['fetched_at'] ?? 0)) < GITHUB_CACHE_TTL) {
            return $cached;
        }
    }
    ...
    $ctx = stream_context_create([
        'http' => [
            'method'  => 'GET',
            'header'  => "User-Agent: OEM-Activation-System\r\nAccept: application/vnd.github+json\r\n",
            'timeout' => 15,
            'ignore_errors' => true,
        ],
    ]);
    ...
    if ($httpCode === 403) {
        throw new Exception('GitHub API rate limit exceeded. Try again in a few minutes.');
    }
    if ($httpCode === 404) {
        throw new Exception('No releases found in repository');
    }
    if ($httpCode !== 200) {
    ...
    // Find upgrade package asset (ZIP files matching upgrade pattern)
    $upgradeAsset = null;
    foreach ($data['assets'] ?? [] as $asset) {
        $name = strtolower($asset['name'] ?? '');
        if (str_ends_with($name, '.zip') &&
            (str_contains($name, 'upgrade') || str_contains($name, 'release') || str_contains($name, 'update'))) {
            $upgradeAsset = [
                'name'           => $asset['name'],
                'size'           => $asset['size'],
                'download_url'   => $asset['browser_download_url'],
    ...
    // If no upgrade-specific asset, look for any ZIP
    if (!$upgradeAsset) {
        foreach ($data['assets'] ?? [] as $asset) {
            if (str_ends_with(strtolower($asset['name'] ?? ''), '.zip')) {
                $upgradeAsset = [
...
function parseVersionFromTag(string $tag): string {
    // Strip v prefix: v2.1.0 → 2.1.0
    return ltrim($tag, 'vV');
}

// ── 0a. Check GitHub for Updates ────────────────────────────
...
function handle_upgrade_check_github(PDO $pdo, array $admin_session, $json_input): void {
    ...
    try {
        $ghData = fetchGitHubRelease($forceRefresh);
    } catch (Exception $e) {
        jsonResponse([
            'success'          => false,
            'update_available' => false,
    ...
    $current = getCurrentVersion();
    $latestTag = $ghData['release']['tag'] ?? '';
    $latestVersion = parseVersionFromTag($latestTag);
    $updateAvailable = version_compare($latestVersion, $current['version'], '>');
    ...
    jsonResponse([
        ...
        'current_version'      => $current['version'],
        'latest_version'       => $latestVersion,
        'release'              => $ghData['release'],
        'asset'                => $ghData['asset'],
        'has_upgrade_package'  => $ghData['asset'] !== null,
    ]);
}

// ── 0b. Download from GitHub ────────────────────────────────
...
function handle_upgrade_download_github(PDO $pdo, array $admin_session, $json_input): void {
    ...
    $downloadUrl = $json_input['download_url'] ?? '';
    $assetName   = $json_input['asset_name'] ?? '';

    if (empty($downloadUrl)) {
        jsonResponse(['success' => false, 'error' => 'Download URL is required']);
        return;
    }

    // Validate URL is from GitHub
    if (!preg_match('#^https://github\.com/.+/releases/download/.+\.zip$#i', $downloadUrl)) {
        jsonResponse(['success' => false, 'error' => 'Invalid GitHub download URL']);
        return;
    }

    // Check no active upgrade
    $stmt = $pdo->prepare("SELECT id, status FROM upgrade_history WHERE status NOT IN ('completed', 'failed', 'rolled_back') LIMIT 1");
    $stmt->execute();
    $active = $stmt->fetch();
    if ($active) {
        jsonResponse(['success' => false, 'error' => "Another upgrade (#{$active['id']}) is in progress"]);
        return;
    }
    ...
    $tmpFile = tempnam(sys_get_temp_dir(), 'oem_upgrade_');
    $response = @file_get_contents($downloadUrl, false, $ctx);
    if ($response === false) {
        @unlink($tmpFile);
        jsonResponse(['success' => false, 'error' => 'Failed to download from GitHub']);
        return;
    }
    ...
    if ($manifestJson === false) {
        $zip->close();
        @unlink($tmpFile);
        jsonResponse(['success' => false, 'error' => 'ZIP missing manifest.json']);
        return;
    }
    ...
    // Store the package
    $checksum = hash_file('sha256', $tmpFile);
    $storedName = $assetName ?: "upgrade_{$manifest['version']}_{$checksum}.zip";
    $storedPath = getUpgradeDir() . '/' . $storedName;
    rename($tmpFile, $storedPath);
