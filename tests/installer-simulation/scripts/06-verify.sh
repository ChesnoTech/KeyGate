...
#
# Runs 30+ automated checks covering:
#   - API health endpoint
#   - Admin authentication
#   - Database integrity (tables, migrations, config)
#   - Trusted network auto-detection
#   - File system artifacts
#   - Security (installer lock, demo removal)
#   - Client API endpoints (PowerShell UA required)
# ═══════════════════════════════════════════════════════════════════
...
run_test() {
    ...
    if [ "$result" -eq 0 ]; then
        ...
        log INFO "  [PASS] TEST ${TOTAL_TESTS}: ${name}"
    else
        FAIL_COUNT=$((FAIL_COUNT + 1))
        log ERROR "  [FAIL] TEST ${TOTAL_TESTS}: ${name}"
    fi
}
...
# Restart Apache to ensure clean state (clear opcache)
...
# Verify admin credentials work via direct DB check
...
for dir in "api" "controllers" "functions" "install" "database"; do
    run_test "Directory '${dir}/' exists" $([ -d "${WEBROOT}/${dir}" ] && echo 0 || echo 1)
done
...
human_action "Verifying installer is locked..."
...
human_action "Testing client API endpoints..."
...
# Test health endpoint via PowerShell UA too
HEALTH_PS=$(curl -s -o /dev/null -w '%{http_code}' \
    -H "User-Agent: ${PS_UA}" \
    "${BASE_URL}/api/health.php" 2>/dev/null || echo "000")
run_test "API /api/health.php via PowerShell UA (HTTP ${HEALTH_PS})" $([ "$HEALTH_PS" = "200" ] || [ "$HEALTH_PS" = "503" ] && echo 0 || echo 1)
...
if [ $FAIL_COUNT -eq 0 ]; then
    ...
    echo -e "  ${BOLD}${RED}${FAIL_COUNT} TEST(S) FAILED${NC}" | tee -a "$LOG_FILE"
    echo "" | tee -a "$LOG_FILE"
fi
